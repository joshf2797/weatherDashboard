...
// Function to get and display the current weather conditions
...
const getFiveDayForecast = () => {
    ...
    let weatherURL = "https://api.openweathermap.org/data/2.5/forecast?q=" + city + "&units=imperial" + "&APPID=" + owmAPI;
    // Fetch from API
    fetch(weatherURL)
        .then (handleErrors)
        .then((response) => {
...
const renderCities = () => {
    ...
    if (localStorage.length===0){
        if (lastCity){
            ...
            $('#search-city').attr("value", "Phoenix");
        }
    } else {
        // sets key in localstorage to cities
        let lastCityKey="cities"+(localStorage.length-1);
        lastCity=localStorage.getItem(lastCityKey);
        // set search input to last city user searched
        $('#search-city').attr("value", lastCity);
        // appends stored cities to page
        for (let i = 0; i < localStorage.length; i++) {
